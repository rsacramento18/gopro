...
func main() {
	...
	fmt.Println("======================")
	fmt.Println("1 - Scan Devices")

	fmt.Scanf("%d", &operation)
	fmt.Println("this was the number", operation)
	...
	if len(ct.Scans) > 0 {
		fmt.Println("Connect to device:")
		fmt.Println("==================")
		ct.PrintScans()
		fmt.Scanf("%d", &operation)

		ct.Connect(ct.Scans[operation])
	}
}
...
// 					shutterOn := []byte{0x03, 0x05, 0x01, 0x01}
